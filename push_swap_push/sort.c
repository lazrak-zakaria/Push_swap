...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/23 11:34:37 by zlazrak           #+#    #+#             */
/*   Updated: 2022/12/23 11:34:39 by zlazrak          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_from_a_to_b(t_list **stack_a, t_list **stack_b, int n, t_utils utils)
{
	...
	while (*stack_a)
	{
		while (*stack_a && utils.start++ <= utils.end)
		{
			...
			while (utils.head->sort_index > utils.end)
				utils.head = utils.head->next;
			while (utils.tail->sort_index > utils.end)
